...
This chapter aligns with, and supplements [the EA Forum post "Marketing messages trial for GWWC Giving Guide Campaign"](https://forum.effectivealtruism.org/posts/nhbeKbwMgFKfrzLNb/marketing-messages-trial-for-gwwc-giving-guide-campaign)
:::
...
**Context**: Facebook advertisements with a range of audiences/filters^[See ['which videos'](#which_videos) below for a list and brief descrription of these audiences.]
...
See discussion of this and other modeling and simulation approaches moved to separate file `modeling_fb_discussion.qmd`]
...
A `r op(prior_click_per_reach*100)`% click rate (as a share of 'reach') seemed like a reasonable mean. This is approximately the rate GWWC saw in all of its trials before February 2021, when this trial started. `logit(prior_click_per_reach)` gives us the intercept associated with this prior expected outcome rate.  
