...
::: {.callout-note collapse="true"}
## Accesing, downloading, inputting data
...
```{r}
( 
  campaign_date_impressions <- gg_campaign_by_ad_by_text_age_gender %>%   
    ...
    summarize(
      ...
      `cost per result` = round(cost/results,1)
      ) %>% 
    arrange(starts) %>% 
...
- Cost per click by age and gender
- Cost per click by audience (non-cause treatments)
- Clicks per impression, Results per impression by ad text and by video
- Cost per result by ad text and by video 
