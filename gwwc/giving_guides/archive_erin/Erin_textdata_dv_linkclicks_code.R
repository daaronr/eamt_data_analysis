...
#### UPLOAD DATA ####
raw_data <- read.csv("textdata_dv_linkclicks.csv")
...
#########  
  ...
  ###### PLOT 1: Cost adjusted DV (link clicks) by text ####
  ...
  data %>% 
    ...
    xlab('Text')+
    ggtitle('Link Clicks/$ spent by Text')+
  ...
  ##### PLOT 2: DV (Link clicks) by text ####
  ##### Questions: Should I have multiplied the DV by 100?
  ...
  ##### PLOT 3: Cost adjusted DV (Link clicks) by text and audience #####
  ...
  data %>% filter(ave.cost.impr>0 & audience !="retargeting") %>%
    ...
    ylab('Link Clicks/$ spent')+
    xlab('Audience')+
    ggtitle('Link Clicks/$ spent by Text and Audience')+
  ...
  ##### PLOT 4: DV (Link clicks) by text and audience #####
  ...
  data %>% filter(ave.cost.impr>0 & audience !="retargeting") %>%
    ...
    ylab('Link Clicks (%)')+
    xlab('Audience')+
    ggtitle('Link Clicks by Text and Audience')+
    theme_apa(legend.font.size = 8,legend.use.title = TRUE)+
    geom_text(aes(label = paste("$",mean_dv %>% round(.,2)), y=5), position = position_dodge(.9), size=4, color="white") +
    theme(text=element_text(size=10)) +
    scale_y_continuous(limits = c(0,1.75), oob = rescale_none, breaks=seq(0,1.75, by=.5)) +
    scale_x_discrete(labels=c("philanthropy","animal"   ,"climate","general","lookalike","poverty"))
