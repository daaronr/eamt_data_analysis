...
The code above inputs and builds 2-4 related data frames (tibbles), which were constructed from the collapsed (aggregated) data by multiplying rows according to observation counts.^[I am not sure where this was done.]

`gwwc_text_clicks`: Observations of link clicks ...  by texts above video
`gwwc_vid_clicks`:  ...  by video content^[I ignore the 'click data' results for now because I don't think it is particularly meaningful.]
